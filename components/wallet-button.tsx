"use client";

import { useEffect, useState } from "react";
import { createPortal } from "react-dom";
import { useAccount, useChainId, useConnect, useDisconnect, useSwitchChain, type Connector } from "wagmi";
import { hskChain } from "@/lib/chains";
import { shortAddress } from "@/lib/utils";
import { Icon } from "@/components/icons";

// Official brand SVG icons
const MetaMaskIcon = () => (
  <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M29.8 4.3l-8.7 7.2-2.1-5.1 10.8-2.1z" fill="#E2761B" />
    <path d="M2.2 4.3l8.7 7.2 2.1-5.1L2.2 4.3z" fill="#E4761B" />
    <path d="M25.3 22.8l-4.2-3.1 3.5-3.5.7 6.6z" fill="#E4761B" />
    <path d="M6.7 22.8l4.2-3.1-3.5-3.5-.7 6.6z" fill="#E4761B" />
    <path d="M19 12.8l2.1-1.3-4.5 1.5 2.4-.2z" fill="#E4761B" />
    <path d="M13 12.8l-2.1-1.3 4.5 1.5-2.4-.2z" fill="#E4761B" />
    <path d="M16 13.5l3.2-4.5h-6.4l3.2 4.5z" fill="#E4761B" />
    <path d="M16 19.5c-1.5 0-2.8-.8-3.5-2l3.5-5.5 3.5 5.5c-.7 1.2-2 2-3.5 2z" fill="#E4761B" />
    <path d="M21.1 11.5l8.7-7.2-3.6 8.7-5.1-1.5z" fill="#D7C1B3" />
    <path d="M10.9 11.5L2.2 4.3l3.6 8.7 5.1-1.5z" fill="#D7C1B3" />
    <path d="M21.1 11.5l5.1 1.5-4.1 3.2-1-4.7z" fill="#FF8F00" />
    <path d="M10.9 11.5l-5.1 1.5 4.1 3.2 1-4.7z" fill="#FF8F00" />
    <path d="M16 19.5l3.5-2 1.6 5.3-5.1-3.3z" fill="#D7C1B3" />
    <path d="M16 19.5l-3.5-2-1.6 5.3 5.1-3.3z" fill="#D7C1B3" />
    <path d="M16 23.5c-2.3 0-4.3-1.1-5.5-2.8l1.6-5.3 3.9 2.5 3.9-2.5 1.6 5.3c-1.2 1.7-3.2 2.8-5.5 2.8z" fill="#FF8F00" />
    <path d="M21.1 16.2l4.2-3.2.7 6.6-4.9-3.4z" fill="#FF8F00" />
    <path d="M10.9 16.2l-4.2-3.2-.7 6.6 4.9-3.4z" fill="#FF8F00" />
  </svg>
);

const PhantomIcon = () => (
  <svg width="28" height="28" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="64" cy="64" r="64" fill="#AB9FF2" />
    <path
      d="M105 64C105 86.6 86.6 105 64 105C41.4 105 23 86.6 23 64C23 41.4 41.4 23 64 23C86.6 23 105 41.4 105 64Z"
      fill="#534BAE"
    />
    <circle cx="49" cy="56" r="6.5" fill="#FFFFFF" />
    <circle cx="79" cy="56" r="6.5" fill="#FFFFFF" />
  </svg>
);

const OKXIcon = () => (
  <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect width="32" height="32" rx="7" fill="#000000" />
    <path d="M7 7h6v6H7V7zm12 0h6v6h-6V7zm-6 6h6v6h-6v-6zm-6 6h6v6H7v-6zm12 0h6v6h-6v-6z" fill="#FFFFFF" />
  </svg>
);

const CoinbaseIcon = () => (
  <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect width="32" height="32" rx="16" fill="#0052FF" />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M16 23.5c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5-7.5 3.358-7.5 7.5 3.358 7.5 7.5 7.5zm0-3c2.485 0 4.5-2.015 4.5-4.5s-2.015-4.5-4.5-4.5-4.5 2.015-4.5 4.5 2.015 4.5 4.5 4.5z"
      fill="white"
    />
  </svg>
);

const RabbyIcon = () => (
  <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect width="32" height="32" rx="7" fill="#8697FF" />
    <circle cx="16" cy="16" r="8" fill="#FFFFFF" />
    <circle cx="13.5" cy="15" r="1.5" fill="#8697FF" />
    <circle cx="18.5" cy="15" r="1.5" fill="#8697FF" />
  </svg>
);

const GenericWalletIcon = () => (
  <svg
    width="28"
    height="28"
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
    className="text-emerald-400"
  >
    <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
    <path d="M12 2v9" />
    <path d="M8 5h8" />
  </svg>
);

const DOWNLOAD_LINKS: Record<string, { name: string; url: string }> = {
  metamask: { name: "MetaMask", url: "https://metamask.io/download/" },
  phantom: { name: "Phantom", url: "https://phantom.app/download" },
  okx: { name: "OKX Wallet", url: "https://www.okx.com/web3" },
  coinbase: { name: "Coinbase Wallet", url: "https://www.coinbase.com/wallet" },
  rabby: { name: "Rabby Wallet", url: "https://rabby.io" },
};

function getWalletIcon(id: string, name: string) {
  const key = `${id} ${name}`.toLowerCase();
  if (key.includes("metamask")) return <MetaMaskIcon />;
  if (key.includes("phantom")) return <PhantomIcon />;
  if (key.includes("okx")) return <OKXIcon />;
  if (key.includes("coinbase")) return <CoinbaseIcon />;
  if (key.includes("rabby")) return <RabbyIcon />;
  return <GenericWalletIcon />;
}

export function WalletButton({ className = "" }: { className?: string }) {
  const { address, isConnected } = useAccount();
  const chainId = useChainId();
  const { connectAsync, connectors, isPending } = useConnect();
  const { disconnect } = useDisconnect();
  const { switchChain } = useSwitchChain();

  const [showModal, setShowModal] = useState(false);
  const [connectingId, setConnectingId] = useState<string | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
  }, []);

  const wrongNetwork = isConnected && chainId !== hskChain.id;

  // Handle wrong network status with reactive one-click switch
  if (mounted && isConnected && wrongNetwork) {
    return (
      <button
        className={`wallet-control border-amber-500/40 text-amber-300 hover:border-amber-400 ${className}`}
        title={`Switch wallet network to ${hskChain.name} (${hskChain.id})`}
        onClick={() => switchChain?.({ chainId: hskChain.id })}
      >
        <span className="status-dot status-warning" />
        <span>Switch to {hskChain.name}</span>
      </button>
    );
  }

  // Connected state
  if (mounted && isConnected) {
    return (
      <button
        className={`wallet-control ${className}`}
        title="Click to disconnect wallet"
        onClick={() => disconnect()}
      >
        <span className="status-dot" />
        <span className="font-mono">{shortAddress(address)}</span>
      </button>
    );
  }

  async function handleConnect(connector: Connector) {
    setErrorMsg(null);
    setConnectingId(connector.uid);

    try {
      // Awakening the browser extension popup directly
      if (typeof window !== "undefined" && (window as unknown as { ethereum?: { request: (args: unknown) => Promise<unknown> } }).ethereum) {
        (window as unknown as { ethereum: { request: (args: unknown) => Promise<unknown> } }).ethereum
          .request({ method: "eth_requestAccounts" })
          .catch(() => {});
      }

      await connectAsync({ connector });
      setShowModal(false);
    } catch (err: unknown) {
      console.warn("Wallet connection attempt:", err);
      const msg = err instanceof Error ? err.message : String(err);
      if (msg.includes("rejected") || msg.includes("User rejected")) {
        setErrorMsg("Connection was rejected in wallet.");
      } else if (msg.includes("not found") || msg.includes("Connector not found") || msg.includes("target")) {
        const idLower = connector.id.toLowerCase();
        const download = DOWNLOAD_LINKS[idLower] || DOWNLOAD_LINKS.metamask;
        setErrorMsg(
          `${connector.name} extension was not found. Please install ${connector.name} or use Browser Extension.`
        );
      } else {
        setErrorMsg(msg);
      }
    } finally {
      setConnectingId(null);
    }
  }

  async function handleDirectBrowserConnect() {
    if (
      typeof window === "undefined" ||
      !(window as unknown as { ethereum?: { request: (args: unknown) => Promise<unknown> } }).ethereum
    ) {
      setErrorMsg("No Web3 wallet extension found. Please install MetaMask, Phantom, or OKX Wallet.");
      return;
    }

    setErrorMsg(null);
    setConnectingId("direct");
    try {
      const ethereum = (window as unknown as { ethereum: { request: (args: unknown) => Promise<unknown> } }).ethereum;
      await ethereum.request({ method: "eth_requestAccounts" });
      const defaultConnector = connectors.find((c) => c.id === "injected") || connectors[0];
      if (defaultConnector) {
        await connectAsync({ connector: defaultConnector });
      }
      setShowModal(false);
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : "Connection failed.");
    } finally {
      setConnectingId(null);
    }
  }

  // Deduplicate connectors by ID
  const displayConnectors = connectors.filter(
    (connector, index, self) => index === self.findIndex((c) => c.id === connector.id)
  );

  return (
    <>
      <button
        className={`button button-primary ${className}`}
        onClick={() => {
          setErrorMsg(null);
          setShowModal(true);
        }}
      >
        <Icon name="wallet" className="h-4 w-4" />
        Connect Wallet
      </button>

      {/* Wallet Selector Modal - Portaled to document.body for guaranteed screen centering */}
      {mounted && showModal && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 overflow-y-auto min-h-screen">
          {/* Backdrop */}
          <div
            className="fixed inset-0 bg-black/80 backdrop-blur-md transition-opacity"
            onClick={() => setShowModal(false)}
          />

          {/* Modal Container */}
          <div className="relative my-auto w-full max-w-md rounded-2xl border border-white/[0.08] bg-slate-950/95 p-6 sm:p-8 shadow-2xl backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-200 z-10 text-left">
            {/* Close Button */}
            <button
              className="absolute right-4 top-4 rounded-lg p-1.5 text-gray-400 hover:bg-white/[0.04] hover:text-white transition-colors"
              onClick={() => setShowModal(false)}
            >
              <Icon name="x" className="h-4 w-4" />
            </button>

            {/* Header Content */}
            <div className="flex flex-col items-center text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-400">
                <Icon name="wallet" className="h-6 w-6" />
              </div>
              <h3 className="text-xl font-extrabold text-white mt-4">Connect Wallet</h3>
              <p className="text-xs text-gray-400 mt-1.5 max-w-xs leading-relaxed">
                Connect your preferred browser wallet to interact with HashGuard on {hskChain.name}.
              </p>
            </div>

            {/* Fast-path Default Extension Trigger */}
            <div className="mt-6">
              <button
                type="button"
                disabled={Boolean(connectingId)}
                onClick={handleDirectBrowserConnect}
                className="w-full flex items-center justify-between rounded-xl border border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 p-3.5 text-left transition-all duration-200 group"
              >
                <div className="flex items-center gap-3">
                  <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-500/20 text-emerald-400">
                    <Icon name="spark" className="h-5 w-5" />
                  </span>
                  <div>
                    <p className="font-bold text-sm text-emerald-400">
                      {connectingId === "direct" ? "Calling Wallet Extension…" : "Direct Browser Extension"}
                    </p>
                    <p className="text-[11px] text-gray-400">Auto-detect MetaMask, Phantom, OKX, Rabby</p>
                  </div>
                </div>
                <Icon name="arrow" className="h-4 w-4 text-emerald-400" />
              </button>
            </div>

            <div className="relative my-4 text-center">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-white/[0.06]" />
              </div>
              <span className="relative bg-slate-950 px-3 text-[10px] uppercase font-bold tracking-wider text-gray-500">
                Or select provider
              </span>
            </div>

            {/* Specific Connector Options */}
            <div className="space-y-2.5 max-h-[280px] overflow-y-auto pr-1">
              {displayConnectors.map((connector) => {
                const isConnecting = connectingId === connector.uid;
                const name = connector.name === "Injected" ? "Default Browser Wallet" : connector.name;

                return (
                  <button
                    key={connector.uid}
                    disabled={Boolean(connectingId)}
                    className="w-full flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.05] hover:border-emerald-500/30 p-3 text-left transition-all duration-200 group disabled:opacity-50"
                    onClick={() => handleConnect(connector)}
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/[0.03]">
                        {getWalletIcon(connector.id, connector.name)}
                      </span>
                      <div>
                        <p className="font-bold text-sm text-white group-hover:text-emerald-400 transition-colors">
                          {name}
                        </p>
                        <p className="text-[10px] text-gray-500">
                          {isConnecting ? "Calling wallet popup…" : "Click to connect"}
                        </p>
                      </div>
                    </div>
                    {isConnecting ? (
                      <span className="text-xs text-emerald-400 font-semibold animate-pulse">Connecting…</span>
                    ) : (
                      <Icon name="arrow" className="h-4 w-4 text-gray-600 group-hover:text-emerald-400 transition-colors" />
                    )}
                  </button>
                );
              })}
            </div>

            {/* Error & Helper Message */}
            {errorMsg && (
              <div className="mt-4 rounded-xl bg-rose-500/10 border border-rose-500/20 p-3 text-xs text-rose-300 leading-relaxed">
                <p>{errorMsg}</p>
                <div className="mt-2 flex flex-wrap gap-2 pt-2 border-t border-rose-500/20">
                  <a
                    href="https://metamask.io/download/"
                    target="_blank"
                    rel="noreferrer"
                    className="underline text-rose-200 hover:text-white"
                  >
                    Get MetaMask
                  </a>
                  <span>·</span>
                  <a
                    href="https://phantom.app/download"
                    target="_blank"
                    rel="noreferrer"
                    className="underline text-rose-200 hover:text-white"
                  >
                    Get Phantom
                  </a>
                  <span>·</span>
                  <a
                    href="https://www.okx.com/web3"
                    target="_blank"
                    rel="noreferrer"
                    className="underline text-rose-200 hover:text-white"
                  >
                    Get OKX
                  </a>
                </div>
              </div>
            )}
          </div>
        </div>,
        document.body
      )}
    </>
  );
}
